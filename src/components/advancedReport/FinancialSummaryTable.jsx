...
const FIXED_COLUMNS = [
  { key: "title", title: "نام پروژه", type: "string", sortable: true, filterable: true, align: "right" },
  { key: "inspector_name", title: "نام بازرس", type: "string", sortable: true, filterable: true, align: "right" },
  { key: "personnel_code", title: "کد پرسنلی", type: "string", sortable: true, filterable: true, align: "right" },
  { key: "rfi_date", title: "تاریخ شروع بازرسی", type: "date", sortable: true, filterable: false, align: "right" },
  { key: "date_shamsi", title: "تاریخ شمسی", type: "string", sortable: true, filterable: false, align: "right" },
  { key: "approve_manday", title: "تعداد روز مورد تایید", type: "number", sortable: true, filterable: false, showSum: true, align: "center" },
  { key: "inspector_price", title: "قیمت بازرسی", type: "number", sortable: true, filterable: false, showSum: true, align: "center" },
  { key: "fix_total_price", title: "جمع کارکرد اعداد ثابت", type: "number", sortable: true, filterable: false, showSum: true, align: "center" },
  { key: "appman", title: "تعداد روز مورد تایید نهایی", type: "number", sortable: true, filterable: false, editable: true, showSum: true, align: "center" },
  { key: "final_price", title: "قیمت بازرس نهایی", type: "number", sortable: true, filterable: false, editable: true, showSum: true, align: "center" },
  { key: "total_price", title: "جمع کارکرد اعداد متغیر", type: "number", sortable: true, filterable: false, showSum: true, align: "center" },
  { key: "remark", title: "توضیحات", type: "string", sortable: true, filterable: true, align: "right" },
];

// تابع محاسبه جمع ستون‌های عددی
...
const FinancialSummaryTable = forwardRef(({ data, onDelete, onEdit, isLoading, isEditingEnabled = true }, ref) => {
  ...
  // محاسبه مقادیر جمع برای هر ستون عددی که showSum دارد
  const columnSums = useMemo(() => {
    const sums = {};
    FIXED_COLUMNS.forEach(column => {
      if (column.showSum) {
        sums[column.key] = calculateColumnSum(filteredAndSortedData, column.key);
      }
    });
    return sums;
  ...
  const getTextAlign = (column) => {
    if (column.align) {
      if (column.align === 'center') return 'text-center';
      if (column.align === 'left') return 'text-left';
      return 'text-right';
    }
    return column.type === 'number' ? 'text-center' : 'text-right';
  };
          ...
          <div dir="rtl" className="min-w-full inline-block align-middle">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gradient-to-r from-blue-600 to-blue-500 sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="px-3 py-2 text-center text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap" style={{ paddingRight: '24px' }}><span>ردیف</span></th>
                  {FIXED_COLUMNS.map((column) => {
                    ...
                    return (
                      <th key={column.key} className={`px-3 py-2 ${textAlign} text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap relative`}>
                        <div className="flex items-center gap-1" style={{ justifyContent: column.align === 'center' ? 'center' : 'flex-start' }}>
                                ...
                                <div className="absolute top-full right-0 mt-1 z-50 bg-white rounded-lg shadow-xl border border-gray-200 min-w-[250px] max-h-[300px] overflow-y-auto">
                                  <div className="p-2 border-b border-gray-200">
                                    <div className="flex justify-between mb-2">
                                      <span className="text-xs font-semibold">جستجو در {column.title}</span>
                  ...
                  <th className="px-3 py-2 text-center text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap"><span>حذف</span></th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAndSortedData.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className={`hover:bg-gray-50 ${isModified ? 'bg-amber-100' : ''}`}>
                      <td className="px-3 py-2 text-center whitespace-nowrap text-xs text-gray-900" style={{ paddingRight: '24px' }}>{index + 1}</td>
                      {FIXED_COLUMNS.map((column) => {
                        ...
                        const textAlign = getTextAlign(column);
                        return (
                          <td key={column.key} className={`px-3 py-2 whitespace-nowrap text-xs text-gray-900 ${textAlign}`} onClick={() => handleCellClick(index, column.key, cellValue, item)}>
                            <div className="flex items-center gap-1" style={{ justifyContent: column.align === 'center' ? 'center' : (column.type === 'number' ? 'flex-start' : 'flex-end') }}>
                              {isEditing ? (
                                <>
                      ...
                      <td className="px-3 py-2 text-center whitespace-nowrap text-xs text-gray-900">
                ...
                {/* ردیف جمع ستون‌های عددی */}
                <tr className="bg-gray-100 border-t-2 border-gray-300 font-bold">
                  <td className="px-3 py-2 text-center whitespace-nowrap text-xs text-gray-900 font-bold" style={{ paddingRight: '24px' }}>جمع کل</td>
                  {FIXED_COLUMNS.map((column) => {
                    const textAlign = getTextAlign(column);
                    if (!column.showSum) {
                      return <td key={`sum-${column.key}`} className={`px-3 py-2 whitespace-nowrap text-xs text-gray-500 ${textAlign}`}></td>;
                    }
                    const sumValue = columnSums[column.key];
                    return (
                      <td key={`sum-${column.key}`} className={`px-3 py-2 whitespace-nowrap text-xs font-bold text-gray-800 ${textAlign} bg-gray-100`}>
                        {new Intl.NumberFormat('fa-IR').format(sumValue)}
                      </td>
                    );
                  })}
                  <td className="px-3 py-2 text-center whitespace-nowrap text-xs text-gray-500 bg-gray-100">-</td>
                </tr>
              </tbody>
            </table>
          </div>
